...
define(["kendo.all.min","docs/DocBase", "docs/mySelObject", "services/proxyService", "services/SessionService","utils",
        "dataSources/pkDocDataSource","dataSources/allUsersDataSource",
        'dataSources/allOtdelsDataSource',
        'dataSources/allShefsDataSource',
        'dataSources/ibFieldsDataSource',
		'dataSources/pkDocsListDataSource'
    ],
function(kendo, DocBase, mySelObject, proxy, sessionService, utils,ibDocsDs,allUsersDs,allOtdelsDs,
         allShefsDs,ibFieldsDs,ibDocsSendedDs){

    var Doc71;
	
    Doc71 = DocBase.extend({
		...
		openFormWindow: function(mode) {
            ...
            console.log(this.dsIbFields._data.length);
            var that = this;
            var url = location.origin + "/gb2/" + this.html_link;
            var patient=this.selIb;
            var title = "Извещение КВД. " + patient.fio;
...
//                _onRequestEnd(0);
        ...
        saveMe: function (iSend) {
			...
			$.ajax({
				url: "default.aspx?action=StacDoct_main/ibDocs_AJAX&action2=71_send_doc&record_id=" + this.recordId +
...
//            if (!this.html_link) {
            ...
            that.dsGetTemplates.read({
                ...
                that.createToolbar();
                if (!that.recordId) {
                    that.dsIbFields.read({ask_id:that.caseId}).then(function(){
                        that.openFormWindow();
                    });
                }
            });
