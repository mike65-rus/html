 ...
 */
define(["kendo.all.min", "docs/DocBase", "services/proxyService", "utils", "alertify", "dataSources/pkDocsListDataSource"],
    function (kendo, DocBase, proxy, utils, alertify, ibDocsDS) {
        ...
        Doc59 = DocBase.extend({
            ...
            dsReadFromSurveyOk: function (e) {
                ...
                //ibDocsDS.read({ ask_id: selIb.id });
				//ibDocsDS.read({ patient_id: selIb.id, date_end: ??? });
            },
            dsReadOk: function (e) {
                if (!e.items) {
                    this.doClose();
...
//                var selIb = proxy.getSessionObject("selectedIb");
                ...
                this.partChooser = $(this.toolbar).find(".part-chooser").first().kendoDropDownList({
                    ...
                    dataSource: [
                        ...
                        { text: "Извещение (090/у)", value: 1 },
                        { text: "Протокол (027-2/у)", value: 2 }
                    ],
            ...
            saveMe: function (iSend) {
                ...
                $.ajax({
                    url: "default.aspx?action=pk/pkDocs_AJAX&action2=59_send_doc&record_id=" + this.recordId +
                        "&evn=" + selIb.evn + "&pin=" + selIb.pin + "&fio=" + selIb.fio + "&doc_id=" + this.docVid.toString() +
