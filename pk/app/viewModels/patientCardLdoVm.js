...
/**
/**
 * This is a LDO part of patientCard
 * @returns {viewModel}
 */
define(['kendo.all.min','kendo-template!views/patientCardLdo','services/proxyService','dataSources/ldoListDataSource',
        'dataSources/periodLdoDataSource','utils'],
    function(kendo,viewId,proxy,ds,dsPeriod,utils) {
        'use strict';
            var gridSelector = "#exams_ldo_grid";
        ...
        var bindWidgets=function() {
            ...
            kendo.bind("#exams-ldo-toolbar",viewModel);
            var grid=$(gridSelector).data("kendoGrid");
...
//                            grid.dataSource.fetch();
        ...
        var doQuery=function(sPin,sDateEnd){
            ...
            $(gridSelector).hide();
            kendo.ui.progress($("#ldo-tab"),true);
            viewModel.gridDs.read({
                ask_id: sPin,
                d1: sDateEnd
            }).
            then(function() {
                    kendo.ui.progress($("#ldo-tab"),false);
                    $(gridSelector).show();
                    var grid=$(gridSelector).data("kendoGrid");
                    if (grid) {
                        grid.resize();
