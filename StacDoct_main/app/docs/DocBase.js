...
define(["kendo.all.min","services/proxyService","services/SessionService",
        ...
        'kendo-template!templates/doc01Phrases'],
    function(kendo,proxy,sessionService,dictofon,auto,note,utils,cadesService,amplify,editTemplateId) {
    "use strict";
...
//                console.log(sText);
        ...
        bindEditor: function() {
            var that=this;
            $(that.content).find('div[contenteditable="true"]').each(function(idx,el) {
                that.bindCurrentEditor(idx,el);
            });
        },
        bindCurrentEditor: function(divIndex,eDiv) {
            var that=this;
            var thisVid=that.docVid;
            var rel=$(eDiv).attr("rel") || "";
            var aTools=[
                {
            ...
            if (thisVid==1) {
                var aOptions=[];
                var templateHtml="<div>"+$("#"+editTemplateId).html()+"</div>";
                $(templateHtml).find("option").each(function(idx,el) {
                    var optRel=$(el).attr("rela") || "";
//                    console.log(optRel);
                    if (optRel && rel) {
                        if (optRel.indexOf(rel)>=0) {
                            aOptions.push(idx);
                        }
                    }
                });
                var newTemplateHtml="";
                if (aOptions.length) {
                    var selectId=that.uuid+"-templateTool1-"+divIndex.toString();
                    newTemplateHtml=" <select  class='template-phrases' style='width:450px;' id='"+selectId+"'>";
                    var allOptions=$(templateHtml).find("option");
                    for (var i=0;i<aOptions.length;i++) {
                        newTemplateHtml=newTemplateHtml+$(allOptions[aOptions[i]])[0].outerHTML;
                    }
                    newTemplateHtml=newTemplateHtml+"</select>";
                    aTools.push(
                        {
                            name: "customTemplate",
...
//                            console.log(e.html);
            ...
            if (thisVid==1 && newTemplateHtml) {
                var el=$("#"+selectId);
                if (el.length) {
                    var options=$(el).find("option");
                    var list = $(el).kendoDropDownList({
                        optionLabel: "Стандартные фразы ...",
                        value: "",
                        change: function (e) {
                            var value = e.sender.value();
                            if (value) {
                                var editor = that.currentEditor;
                                if (editor) {
                                    if (value == "ИМТ") {
                                        var selIb = proxy.getSessionObject("selectedIb");
                                        if (selIb.rost && selIb.ves) {
                                            var imt = utils.calculateImt(selIb.rost, selIb.ves);
                                            if (imt) {
                                                value = "ИМТ=" + imt.toString() + "(кг/кв.м)";
                                            }
                                            else {
                                                value = "";
                                            }
                                ...
                                if (value) {
                                    that.currentEditor.exec("insertHtml", {value: " " + value});
                                }
                            }
                            e.sender.value("");
                        }
                    }).data("kendoDropDownList");
                    var wrapper=list.wrapper.find("span.k-dropdown-wrap").first();
                    if (wrapper.length) {
                        $(wrapper).css("background-color","bisque").css("font-weight","bold").css("font-style","italic");
                        $(wrapper).attr("title","Выбор стандартной фразы для вставки в позицию курсора...");
                    }
                    var ul = list.ul;
                    ul.find("li").each(function (idx, el) {
                        $(el).attr("title",$(options[idx]).attr("value"));
//                        var dataItem = list.dataItem(idx);
                    });
                };
            }
            $(eDiv).on("focus",function(e) {
                that.currentEditor= $(e.target).data("kendoEditor");
            });
        },
        unbindEditor: function() {
            var that=this;
            var listId=that.uuid+"-templateTool1";
            // for select with id startsWith listId
            $("select[id^='"+listId+"']").each(function(idx,el) {
                var kendoList=$(el).data("kendoDropDownList");
                if (kendoList) {
                    kendoList.destroy();
                }
                $(el).remove();
            });
            $(this.content).find("div[contenteditable='true']").each(function(i,el){
