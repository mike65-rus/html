...
function(kendo, DocBase, mySelObject, proxy, sessionService, utils,ibDocsDs,allUsersDs,allOtdelsDs,allShefsDs,ibFieldsDs){

    var Doc66;
	
    Doc66 = DocBase.extend({
        ...
        showCopyDialog: function() {
            ...
            $("<div id='CopyDialog'></div>").kendoDialog({
                ...
                actions:[
                    {text:'Первичный',primary:(this.subType==1),
                    action: function(e) {
                        that.makeCopy(1);
                    }},
                    {text:"При выписке",primary:(this.subType==2),
                    action: function(e) {
                        that.makeCopy(2);
                    }},
                    {text:"Отказ"}
		...
		openFormWindow: function(mode) {
            ...
            var title = "Протокол работы мультидисциплинарной команды. " + patient.fio;
