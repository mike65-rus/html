...
define(["kendo.all.min","docs/DocBase", "docs/mySelObject", "services/proxyService", "services/SessionService","utils",
        ...
        'dataSources/bloodRequestDataSource',
        'models/bloodRequestModel'
    ],
function(kendo, DocBase, mySelObject, proxy, sessionService, utils,ibDocsDs,allUsersDs,allOtdelsDs,allShefsDs,
         bloodRequestDs,BloodRequestModel){

    var Doc60;
	
    Doc60 = DocBase.extend({
                ...
                if (!this.recordId) {
                    ...
                    if (!(selIb.is_ldo)) {
                        aTools.push({
                                type: "button",
                                text: " Изменить",
                                attributes: {"class": "faa faa-file-o survey-edit", "rel": "tooltip", "title": "Изменить"},
                                showText: "both"
                            }
                        );
                    }
		...
		openFormWindow: function(mode) {
            ...
            var title = "Форма заявки на переливание. " + patient.fio;
                            ...
                            that.dsGetById.read({record_id:record_id}).then(function(){
                                ...
                                if (ds._data) {
                                    if (ds._data.length) {
                                        ...
                                        // save into blood_request table
                                        bloodRequestDs.read({record_id:that.recordId}).then(function(){
                                            var brData=bloodRequestDs._data;
                                            var bloodRequestModel;
                                            if (!brData.length) {
                                                bloodRequestModel=new BloodRequestModel();
                                                bloodRequestModel.set("id",null);
                                            }
                                            else {
                                                bloodRequestModel=ds.at(0);
                                            }
                                            bloodRequestModel.set("record_id",that.recordId);
                                            bloodRequestModel.set("ask_id",that.selIb.ask_id);
                                            bloodRequestModel.set("user_id",Number(localStorage['last_user']));
                                            bloodRequestModel.set("patient_name",that.selIb.fio);
                                            bloodRequestModel.set("patient_age",that.selIb.age);
                                            bloodRequestModel.set("niib",that.selIb.niib);
                                            bloodRequestModel.set("otd",that.selIb.otd1);
                                            //
                                            var jsonData=JSON.parse(ds._data[0].json_data);
                                            bloodRequestModel.set("blood_type",jsonData.q_bloodtype_val);
                                            bloodRequestModel.set("rh",jsonData.q_rh_val);
											bloodRequestModel.set("phenotype",jsonData.q_phenotype_val);
											bloodRequestModel.set("indications",jsonData.q_indications_val);
											bloodRequestModel.set("hb",Number(jsonData.q_hb_val.split('_').join('')));
											bloodRequestModel.set("ht",Number(jsonData.q_ht_val.split('_').join('')));
											bloodRequestModel.set("component",jsonData.q_component_val);
											bloodRequestModel.set("volume",Number(jsonData.q_volume_val.split('_').join('')));
											bloodRequestModel.set("purpose",jsonData.q_purpose_val);
											bloodRequestModel.set("doctor",jsonData.q_doctor_val);
											bloodRequestModel.set("direction",jsonData.q_direction_val);
											bloodRequestModel.set("diagnosis",jsonData.q_diagnosis_val);
											bloodRequestModel.set("pregnancy_anamnesis",jsonData.q_pregnancy_val);
											bloodRequestModel.set("transfusion_anamnesis",jsonData.q_transfusions_val);
											bloodRequestModel.set("doses",Number(jsonData.q_doses_val.split('_').join('')));

                                            if (!brData.length) {
                                                bloodRequestDs.add(bloodRequestModel);
                                            }
                                            else {
                                                bloodRequestDs.pushUpdate(bloodRequestModel);
                                                bloodRequestDs.at(0).dirty=true;
                                            }
                                            bloodRequestDs.sync();
                                        });
                                        //
