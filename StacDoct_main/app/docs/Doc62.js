...
define(["kendo.all.min","docs/DocBase", "docs/mySelObject", "services/proxyService", "services/SessionService","utils",
        ...
        'dataSources/allShefsDataSource', "dataSources/ibDocsListDataSource", "alertify"
    ],
function(kendo, DocBase, mySelObject, proxy, sessionService, utils,ibDocsDs,allUsersDs,allOtdelsDs,allShefsDs,ibDocsSendedDs, alertify){

    var Doc62;
	
    Doc62 = DocBase.extend({
        ...
        init: function (data,uuid) {
            ...
            this.docVid = 62;
			this.sendQuestion="Этот документ не был отправлен! Отправить сейчас?";
            this.data = data;
        ...
        createToolbar: function () {
			...
			if (!utils.isViewer()) {
                if (!this.recordId) {
					...
					aTools.push({type: "button", text: " Отправить",
						attributes: { "data-bind":"enabled:surveyId" , "class": "doc-send faa faa-envelope-o", "rel": "tooltip", "title": "Отправить документ" }, showText: "always"}
					);
                    /*
		...
		openFormWindow: function(mode) {
            ...
            var title = "Суицидальный случай. " + patient.fio;
            ...
            var openData={
                ...
                user:{id:oUser.id,name:oUser.name,fio:oUser.fio,role:sessionStorage['last_user_role'],phone:oUser.phone},
...
//                console.log(data);
        ...
        fnOnClick: function (obj) {
            ...
            var fnc = function (e) {
                ...
                if (btnText=="Изменить") {
					if (myObj.isDocumentSended(ibDocsSendedDs)) {
						alertify.alert("Документ уже отправлен.<br>Редактирование невозможно!");
						return;
					}
...
//            this.storeDateTime();
		...
		saveMe: function (iSend) {
			if (this.isDocumentSended(ibDocsSendedDs) && iSend) {
                    kendo.alert("Документ уже был отправлен!")
                    return;
                }
			var myObj = this;
			var myData = myObj.content[0].innerHTML;
			kendo.ui.progress($(this.toolbar),true);
			$.ajax({
				url: "default.aspx?action=StacDoct_main/ibDocs_AJAX&action2=62_send_doc&record_id=" + this.recordId +
